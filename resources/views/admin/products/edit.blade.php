        ...
        <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="subtitle" id="subtitle" value="{{ old('subtitle', $product->subtitle) }}"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                           required>
                    @error('subtitle')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                ...
                <div>
                    ...
                    <input type="number" name="weight_kg" id="weight_kg" value="{{ old('weight_kg', $product->weight_kg) }}" min="0" step="0.1"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    @error('weight_kg')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="md:col-span-2">
                    ...
                    @if($product->illustration)
                        <div class="flex items-center space-x-4">
                            <img src="{{ url('uploads/' . $product->illustration) }}" 
                                 alt="{{ $product->name }}" 
                                 class="w-20 h-20 object-cover rounded-lg border border-gray-300">
                            <span class="text-sm text-gray-600">{{ $product->illustration }}</span>
                        </div>
                    @else
                ...
                <!-- Nouvelle image -->
                <div class="md:col-span-2">
                    <label for="illustration" class="block text-sm font-medium text-gray-700 mb-2">Nouvelle image</label>
                    <input type="file" name="illustration" id="illustration" accept="image/*"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    @error('illustration')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                    <p class="text-sm text-gray-500 mt-1">Laissez vide pour conserver l'image actuelle. Formats: JPEG, PNG, JPG, GIF. Taille max: 2MB</p>
                    <p class="text-sm text-gray-500">La nouvelle image sera enregistrée dans: public/uploads/</p>
                </div>

                <!-- Aperçu de la nouvelle image -->
                <div class="md:col-span-2">
                    <div id="image-preview" class="mt-2 hidden">
                        <p class="text-sm text-gray-700 mb-2">Aperçu de la nouvelle image:</p>
